...
class Pendaftaran_Online_Controller extends Controller
{
    ...
    public function __construct(Pcare_Controller $PcareController , Pasien_Api_Controller $pasien_api, Satusehat_Controller $SatusehatController)
    {
        $this->pcare = $PcareController;
        $this->pasien_api = $pasien_api;
        $this->satusehat = $SatusehatController;
    }
    ...
    public function monitor_bpjs(Request $request)
    {
        try {
            ...
            // ✅ Cek apakah pasien sudah daftar BPJS di tanggal yang sama (tanpa jam)
            ...
            if ($sudahDaftar) {
                return response()->json([
                    'success' => false,
                    'message' => 'Pasien yang Terhormat anda sudah terdaftar pada hari ini di bpjs'
                ], 409);
            }
            ...
            $response = $this->pcare->post_ws_antria_bpjs($databpjs);
     ...
     public function pasiensadd(Request $request)
    {
        try {
            ...
            try {
                $kodeihs = $this->satusehat->get_nik_satusehat($request->nik)->getData(true);
            } catch (\Exception $e) {
                $kodeihs = null;
            }
