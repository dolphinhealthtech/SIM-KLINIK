...
{{-- modal Add Role --}}
<div class="modal fade" id="addjenis_dietModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalLabel">Tambah jenis diet</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="addFormjenis_diet" action="{{ route('jenis_diet.store') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-sm-12">
...
<!-- Modal Import -->
<div class="modal fade" id="importjenis_dietModal" tabindex="-1" role="dialog" aria-labelledby="importjenis_dietModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('jenis_diet.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
...
<script>
        $(document).ready(function() {
            ...
            }).buttons().container().appendTo('#jenis_diettabel_wrapper .col-md-6:eq(0)');
        });

        $('#addFormjenis_diet').on('submit', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.edit-data-jenis_diet', function() {
            var id = $(this).data('id');
            var nama = $(this).data('nama-jenis_diet');

            $('#jenis_dietid_edit').val(id);
            $('#nama_edit').val(nama);
             // Pastikan rhesus terpilih dengan benar
        });

        $('#editFormjenis_diet').on('submit', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.delete-data-jenis_diet', function() {
            let id = $(this).data('id');
            let name = $(this).data('nama-jenis_diet');
        ...
        $('#deleteFormjenis_diet').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#deletejenis_dietModal').modal('hide');
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: response.message,
